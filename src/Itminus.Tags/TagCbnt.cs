...
/// <summary>
/// 代表一组测点组合，且底层通道必须是 <see cref="IContinuousBytesBasedTagChannel"/>。<br/>
/// 为了避免被误用到非<see cref="IContinuousBytesBasedTagChannel"/>场景，这个类被定义成一个内部类。
...
internal class TagCbnt : ITagCbnt
{
    ...
    public virtual async Task ReadAsync(CancellationToken ct)
    {
        ...
        if(channel is null)
        {
            throw new NotImplementedException($"通道组合({nameof(TagCbnt)})依赖于通道{nameof(IContinuousBytesBasedTagChannel)}，但当前实际通道是{channel0.GetType().Name}，如有必要，请考虑提供自己的通道组合实现。当前测点组合名称={this.TagName()}");
        }
